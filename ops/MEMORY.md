...
| Datum | Was | Warum gut |
|-------|-----|-----------|
| 2026-04-04 | Sprint 25 Phantom-Open (S25-2+S25-3): Dungeon + NPC-Gedächtnis waren bereits implementiert, SPRINT.md zeigte Drift | Parallel-Sessions schreiben SPRINT.md unabhängig. Jede Session muss git fetch + SPRINT.md lesen bevor sie Items als offen wertet. |
| 2026-04-04 | Wu-Xing NPC-Events: Events wurden emittiert (element:fire etc.) aber niemand hörte zu | Publish ohne Subscribe = totes Feature. Bus-Events immer mit grep auf .on()-Handler prüfen bevor man sie als "implementiert" markiert. |
...
| 2026-04-04 | S25-2 war schon in `4cf2646` implementiert (Dungeon-Dialog, cave-click-handler), aber SPRINT.md war nicht aktualisiert. Divergenz erkannt beim Grep. | Nach jedem Feature-Commit sofort SPRINT.md updaten — nicht erst beim nächsten Standup. |
| 2026-04-04 | NPC-Daten (5 Konstanten) aus game.js in `npc-data.js` extrahiert. window.INSEL_NPC_* Pattern, IIFE, keine Arrow-Functions (Compat). game.js: −63 LOC. | Zellteilung funktioniert solange keine globalen Zirkelabhängigkeiten entstehen. Script-Reihenfolge in index.html ist der kritische Punkt. |
