...
|-------|-----|-------|---------|
| 2026-04-03 | Remote hatte feat/sprint-25 bereits mit Sprint 26 Done. Session hat Sprint 25 neu geplant → Rebase-Konflikt. | `git fetch` passiert beim Session-Start, aber Branch-Status nicht geprüft vor Sprint Planning. | Vor Sprint Planning: `git log origin/feat/sprint-N --oneline -5` prüfen. Sprint-Nummer aus Remote-SPRINT.md lesen, nicht aus lokalem Zustand. |
| 2026-04-03 | Sprint 27: soundPaletteNote + maybeDungeonEntry. Remote hatte S26 Long-press Palette anders implementiert — beide Ansätze auf Branch. | Zwei Palette-Instrument-Implementierungen gleichzeitig aktiv (Long-press + Pentatonik-Ton). | Oscar hört bei jedem Material-Klick einen melodischen Ton (Pentatonik) UND Long-press spielt Genre-Ton. Kein Konflikt, additiv. |
