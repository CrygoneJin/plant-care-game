...
(function () {
    ...
    // === GENERATIVE NPC-KOMMENTARE → npc-voices.js (#11 Zellteilung) ===
    const { NPC_VOICES, MAT_ADJECTIVES, REACTIONS, TEMPLATES, STREAK_COMMENTS } = window.INSEL_NPC_VOICES;
    ...
    // Zufalls-Insel generieren — Logik in island-gen.js
    function generateStarterIsland() {
        window.INSEL_ISLAND_GEN.generateStarterIsland(grid, ROWS, COLS, MATERIALS);
        window.grid = grid;
    }

    // Lummerland — handgebaute Tutorial-Insel — Logik in island-gen.js
    function generateLummerland() {
        window.INSEL_ISLAND_GEN.generateLummerland(grid, ROWS, COLS, MATERIALS);
        window.grid = grid;
    }
