...
(function () {
    ...
    function addToInventory(material, count) {
        ...
        // Goldener Pfad — erstes Boot: einmaliger Hint
        const isFirstBoat = material === 'boat' && !inventory['boat'] && !localStorage.getItem('insel-boat-hint-shown');
        ...
        if (isFirstBoat) {
            localStorage.setItem('insel-boat-hint-shown', '1');
            setTimeout(() => showToast('⛵ Du hast ein Boot! Stell dich ans Meer — die Ferne ruft! 🌅', 5000), 600);
        }
    }
    ...
    // --- Grid initialisieren (grid.js) ---
    function initGrid() {
        grid = window.INSEL_GRID.initGrid(ROWS, COLS);
        window.grid = grid;
    }

    function generateStarterIsland() {
        window.INSEL_GRID.generateStarterIsland(grid, ROWS, COLS, MATERIALS);
        window.grid = grid;
    }

    // === LUMMERLAND — handgebaute Tutorial-Insel (grid.js) ===
    function generateLummerland() {
        window.INSEL_GRID.generateLummerland(grid, ROWS, COLS, MATERIALS);
        window.grid = grid;
    }
    ...
    function draw() {
        ...
        // Goldener Pfad — Jim Knopf #54: Boot im Inventar + nah am Wasserrand
        if (playerPos && inventory['boat'] > 0 && !prefersReducedMotion) {
            const pr = playerPos.r, pc = playerPos.c;
            const nearEdge = pr < 3 || pr >= ROWS - 3 || pc < 3 || pc >= COLS - 3;
            if (nearEdge) {
                const pulse = 0.35 + 0.25 * Math.sin(time * 2.5);
                const dirs = [];
                if (pr < 3) dirs.push({ dr: -1, dc: 0 });
                if (pr >= ROWS - 3) dirs.push({ dr: 1, dc: 0 });
                if (pc < 3) dirs.push({ dr: 0, dc: -1 });
                if (pc >= COLS - 3) dirs.push({ dr: 0, dc: 1 });
                for (const dir of dirs) {
                    for (let d = 1; d <= 5; d++) {
                        const wr = pr + dir.dr * d;
                        const wc = pc + dir.dc * d;
                        const wx = (wc + WATER_BORDER) * CELL_SIZE;
                        const wy = (wr + WATER_BORDER) * CELL_SIZE;
                        ctx.globalAlpha = pulse * Math.max(0, 1 - d * 0.18);
